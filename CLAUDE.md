...
- **Зависимости объявлены**: `fastmcp>=2.0` — в основных, `anyio>=4.0` — в тестовых (`pytest-anyio` не нужен: маркер `anyio` регистрирует плагин, встроенный в сам `anyio`). Осталось починить маркер платформы у `comtypes` и поднять `requires-python` — см. спецификацию разделения репозиториев.
