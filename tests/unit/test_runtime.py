...
def test_com_threaded_times_out_instead_of_hanging(monkeypatch):
    ...
    with pytest.raises(ToolError, match="не ответил") as excinfo:
        slow()

    # Рецепт обязан быть исполним: одного перезапуска mmain.exe мало — флаг
    # `connected` не отражает живучесть COM, и повтор без `disconnect`
    # подхватил бы тот же мёртвый прокси.
    text = str(excinfo.value)
    assert "mmain.exe" in text
