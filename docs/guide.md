...
```python
sig = prj.signal("имя_сигнала")    # имя — из списка сигналов проекта
...
Имена сигналов можно получить через `list_signals()` или из БД сигналов
(см. SDB в SimInTech). Типичные сигналы: выход блока по имени блока.
