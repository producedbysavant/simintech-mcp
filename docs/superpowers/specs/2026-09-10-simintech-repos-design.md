...
|---|---|---|
| `sdb_adapter.py` (315) | → `simintech-code`, `simintech_api/sdb.py` | доступ к БД сигналов (Category → Group → Signal) в библиотеке отсутствует |
...
- `get_simulation_data` и чтение внутренних сигналов — отдельная задача
  (см. `docs/roadmap-agentic-ecosystem.md` §6.2). Разделение репозиториев её не
  решает.
