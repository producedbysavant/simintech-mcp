...
числа) **отбрасывается**. Иначе обрывок попадал бы в статистику как настоящее
измерение — и ошибка была бы не в точности, а в порядке величины: `1.5e-3`,
обрезанное до `1.5`, даёт тысячекратно неверное значение.
"""
...
class NumericTable(NamedTuple):
    ...
    Именованные поля, а не кортеж: значений пять, и два из них — целочисленные
    счётчики одного типа (`skipped` и `too_wide`), поэтому позиционная
    распаковка молча путала бы «пропущено» с «широких», меняя смысл примечания
    в ответе.
    """
...
def _read_numeric_table(data: bytes, truncated: bool) -> NumericTable:
    ...
    # середину числа. Разбирать её нельзя: `1.5e-3`, обрезанное до `1.5`,
    # попало бы в min/max/среднее/наклон как настоящее измерение — с ошибкой
    # не в точности, а в тысячу раз. Признак неполноты — отсутствие перевода
    # строки в конце прочитанного.
